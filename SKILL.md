...
reference is right and the path has a bug. Authority is by layer: a
reference decides a domain rule, a path file decides ordering and its stop
condition, and the *It ends when* column below only summarises the path.
