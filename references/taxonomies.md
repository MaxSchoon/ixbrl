...
accepted by EDGAR on **17 March 2025** (Release 25.1; FASB announced the
acceptance on 18 March). The SEC concurrently accepted
...
**Current version:** **Solvency II 2.8.2** is the version in production
(applicable from the Q4 / annual 2024 reference periods until Q4 / annual
2026 included); **2.10.0 Final** was published 3 July 2026 for reference
periods from Q1 2027, and **IRRD 2.11.0 Final** on 27 July 2026. "2.9.1
PWD" appears on the EIOPA page only as a change-log baseline. EIOPA
remains xBRL-XML; the xBRL-CSV switch above is an EBA rule. Per-release
table: `references/dpm.md` § DTS and vintages. Pension fund (IORP) reporting moved from 2.7.1
...
**Issuer / Jurisdiction:** Finnish limited companies (osakeyhtiö) and co-operatives (osuuskunta) file statutory financial statements with the **Finnish Trade Register** (kaupparekisteri), operated by **PRH** (Patentti- ja rekisterihallitus), as a **digital financial statement** (digitaalinen tilinpäätös / *digitilinpäätös*): Inline XBRL in XHTML, filed as **XHTML alone or a plain ZIP of XHTML** (PRH/1087/01/2026 s. 2 permits either; notification max 200 MB), **not** an `.xbri` report package, which PRH cannot receive. PRH's sustainability-reporting guidance states the ZIP form; ytj.fi takes the required documents as one XHTML file, the REST interface takes one attached iXBRL file, and ESEF filers submit the ESEF ZIP. Statutory basis: Accounting Act (Kirjanpitolaki 1336/1997) ch. 7 ss. 22–24 and 24a; operative technical requirements in two PRH decisions **PRH/1087/01/2026** (technical filing) and **PRH/1088/01/2026** (PRH identifiers / taxonomy adoption), both in force 24.6.2026.

**Two-stage mandate:** digital (XHTML) filing has been **mandatory for sustainability-reporting (CSRD-scope) companies since FY2025**; structured **taxonomy markup** (PRH identifiers) is mandatory only for **periods starting on/after 1 Jan 2026**. FY2025 permitted untagged XHTML. **Law 555/2026** (in force 30.6.2026) narrowed CSRD scope to a company or group parent that had turnover >€450M **and** on average >1,000 employees **in both the last completed and the immediately preceding financial year**; one qualifying year does not bring a company into scope. A company below the thresholds may commit voluntarily to ESRS reporting, which triggers the same duties. The new scope applies to periods starting on/after 1.7.2026 and may be applied to periods starting on/after 1.1.2026. Filing is voluntary "for the time being" for all other limited companies.
