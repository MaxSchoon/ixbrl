...
RM 2025 § 2.10: "it's mandatory to include the auditor's report
...
|---|---|---|
| Controleverklaring travels as | visible narrative **inside** the annual-report XHTML, under Other information | its **own XBRL instance document**, excluded from the annual-report instance (Ch. 3 art. 4 para. 1) |
...
| Electronically signed? | **no.** FAQ § 2.6.2: not yet required | yes, XAdES-EPES per ETSI TS 101 903 V1.4.1 (Ch. 3 art. 8, Annex II), as a detached XML file |
| Presence asserted by | `kvk:AuditorsReportFinancialStatementsPresent` in the filing-information sidecar | the same mandatory element set |
| Documents filed | 2 (annual report + KvK sidecar) | 3: annual-report instance, auditor's-report instance, signature XML (Ch. 3 art. 7 footnote) |
...
2:403 and consolidated-ESEF examples. Across all of them:
...
So the folded, untagged, two-document shape is not one filer's choice.
It is what the regulator's own reference set does in every size class.
...
`periodType="duration"`, and RTS Annex II point 3 Table 2 lists it among
...
requires `kvk-{date}-{lang}.{extension}`, and the manual states the
...
finds it in **no presentation network at all**; in the NL-GAAP entry
point it has a single definition edge, a domain-member arc under
`kvk:NonDimensionalLineItems`, and in the other two it has no edges. No
...
signature "may be implemented in the future", and RTS Annex II point 2
intends block-tagging of the management report, the notes **and the
other information** (which is where the controleverklaring sits) for
financial years beginning on or after a year the RTS still prints as a
placeholder, pending ESMA's revised block-tagging approach. Nor may a
filer get there early: RTS Annex II point 2 also states that for
financial years beginning before 1 January 2026, applying block-tagging
on a voluntary basis is not permitted.
...
DTS carries 142 concepts, 19 of them tuples, typed as
`string1000ItemType` and `formattedExplanationItemType`: the
...
- When the flag is `Ja`, the controleverklaring is **visibly present**
  ...
  search finds nothing and proves nothing. A flag reading `Ja` over an
  annual report with no controleverklaring is a real defect; an
  untagged controleverklaring is not.
...
consolidation exemption), the package is **not** the shape in
*The auditor's report (controleverklaring) in the package* /
*First: which Nederlandse Taxonomie applies?* above. Verified against
...
`--disclosureSystem NL-INLINE-2025-GAAP-OTHER-PREVIEW`
(see *Validation: offline DTS resolution and `nltaxonomie.nl`*).

<a id="profile-kvk-408-foreign-group-head-pdf"></a>

## Profile: art. 2:408 foreign group head report by PDF e-mail (before FY2028)

**Timing (bi-temporal; see *Bi-temporal cheatsheet (which rule applied
when)*).** The 403 iXBRL route is operative for FY2025 and later. The
**408 route has slipped by two years**: per the SBR-domein
Handelsregister FAQ of 10 July 2026 (Q2.2.4), an art. 2:408
foreign-group-head report for a financial year beginning **before
2028-01-01** is deposited **by PDF e-mail in Pdf-formaat**, and the
...
KvK NL-GAAP filings are catalogued in *Two taxonomy trees, same prefix
family*; verify each tagged fact's prefix against that table. Use `arelleCmdLine ... --saveInstance` to
...
1. **Pin the regime, period, NT generation, and entry point.**
...
4. **Run validation in the operative profile, offline.**
   *Validation: offline DTS resolution and `nltaxonomie.nl`*. Capture
...
5. **Classify each finding.** Route by code prefix using `SKILL.md`'s
   ...
   warnings.
6. **Concept-binding pass.** *Recurring Dutch concept choices that are
   syntactically valid but wrong*. Confirm every fact's QName resolves
   in the operative DTS. `missingReferences` is harder to fix than
   surface validation errors and changes the meaning of every
   downstream check.
7. **Per-scope value pass.**
   *Per-scope value-correctness: what Arelle won't catch*. For
   consolidated + separate filings, sample a dozen line items and confirm the value belongs in the
   scope it appears in. No validator catches this for you.
8. **Sign and period class.** *Sign and balance: the Dutch flavour*,
...
- **Digital-reporting programme: SBR, run by Logius.** Logius manages the
  ...
  gateway). Governance is public-private: the **SBR Beraad** (decision
  board, chaired by the Rijksregisseur), the **SBR Platform**, and the
...
- **Accounting basis is EU-anchored.** Art. 2:362 BW permits **IFRS as
  ...
  to Annex II point 3 metadata tagging; see *What the RTS requires when
...
cites, or a code not listed in *Recurring KvK deposit-blocker patterns* or
*FR-NL- / FG-NL- (SBR Filing Rules / Filing Guidelines)*, say so and
link the primary
